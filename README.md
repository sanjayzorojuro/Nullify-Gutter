...
Automatic Folder Creation: Creates destination directories only if they do not already exist ({ recursive: true } is used).
...
The script is executed using the Node runtime. By default, it operates on the path set within the script (X:\Nullify-gutter).
...
If the X:\Nullify-gutter folder contains:
...
After running node index.js, the folder structure will be:
...
Create a new feature branch (git checkout -b feature/new-logic).

Commit your changes (git commit -m 'feat: improve file filtering').
